...
- `src/lib/analytics.ts` — thin wrapper around PostHog (`capture()`, `initClickTracking()`). See "Analytics" below.
- `src/env.d.ts` — types `import.meta.env`, currently just `PUBLIC_POSTHOG_KEY`.
...
Two tags run in `Layout.astro`'s `<head>`: GTM (`SITE.gtmId`, always on) and PostHog (prod-only, see below). Neither should ever be called directly from application code — GTM goes through `dataLayer.push`, PostHog goes through `src/lib/analytics.ts`'s `capture()`.
...
**Instrumenting a CTA**:
- Static components (`Header`, `Hero`, `CtaBand`, `Pricing`, `Footer`, `Contact` — none hydrated except `Hero`) can't attach a React `onClick`. Add `data-analytics-event="…"` and `data-analytics-prop-<key>="…"` attributes to the element instead; a single delegated listener (`initClickTracking()`, wired once in `Layout.astro`'s `<body>`) picks them up on click. `Footer.tsx`'s link array carries an `analytics: { event, props }` field per item precisely so this stays data-driven — no duplicated JSX.
- Hydrated components with state already in scope (`FAQ`, `PastGlimpse`) call `capture(event, props)` directly from the handler.

Current event table: `ticket_cta_clicked` (`location`, `tier`?), `nav_link_clicked` (`target`), `contact_email_clicked` (`location`, `topic`), `social_link_clicked` (`network`), `faq_item_opened` (`question`), `video_recap_played` (`video`). Autocapture covers everything else without further work.
