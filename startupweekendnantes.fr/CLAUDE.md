...
- `src/components/Picture.tsx` — shared image helper: renders `<picture>` with auto `avif`/`webp` `<source>`s and `width`/`height` from `src/data/image-manifest.ts` (uses `display: contents`, so it adds no layout box). Use it for content images; decorative CSS backgrounds (Hero/Welcome) and the preloaded LCP image stay as plain `<img>`.
- `src/data/image-manifest.ts` — AUTO-GENERATED by `scripts/optimize-images.mjs` (image path → size + which modern formats exist). Do not edit by hand; excluded from eslint.
