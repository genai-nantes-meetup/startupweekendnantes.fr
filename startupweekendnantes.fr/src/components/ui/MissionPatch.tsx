...
export default function MissionPatch({ n, size = 160, className }: Props) {
  ...
  const botId = `mb-${n}`;
  return (
    <svg width={size} height={size} viewBox="0 0 260 260" className={className} aria-label={`Patch mission ${mm}`} role="img">
      <defs>
      ...
      <text fontFamily="'Space Mono',monospace" fontSize="11.5" fontWeight="700" fill={s.top} letterSpacing="0.4">
        <textPath href={`#${topId}`} startOffset="50%" textAnchor="middle">STARTUP WEEKEND NANTES</textPath>
      </text>
      <text fontFamily="'Space Mono',monospace" fontSize="12.5" fontWeight="700" fill={s.accent} letterSpacing="3.5">
        <textPath href={`#${botId}`} startOffset="50%" textAnchor="middle">{`MISSION ${mm} · 2026`}</textPath>
      </text>
      <ellipse cx="130" cy="126" rx="44" ry="16" fill="none" stroke={s.accent} strokeWidth="2.5" transform="rotate(-20 130 126)" />
      ...
      <text x="130" y="166" textAnchor="middle" fontFamily="'Space Mono',monospace" fontSize="11" fontWeight="700" fill={s.accent} letterSpacing="2">{pb}</text>
    </svg>
  );
}
