/**
 * /llms.txt — digest lisible par les moteurs de réponse IA (ChatGPT, Perplexity,
 * Google AI Overviews, Claude). Suit la convention llmstxt.org : un H1, un résumé
 * en blockquote, puis des sections factuelles en texte brut citables verbatim.
 *
 * Généré depuis les mêmes modules `src/data/` que le site (aucune duplication),
 * donc il roule avec l'édition automatiquement — ne jamais coder en dur une date/prix ici.
 */
...
function build(): string {
  ...
  lines.push(
    `> ${EDITION.editionNumber}ᵉ édition du Startup Weekend Nantes : 54 heures pour transformer ` +
      `une idée en startup, en équipe, à Nantes. ${dates}, ${venue.name}, ${venue.addressLocality}.`
  );
  lines.push('');
  lines.push(
    `Le Startup Weekend Nantes est un week-end d'entrepreneuriat ouvert à tous (étudiants, ` +
      `développeurs, designers, marketeurs, porteurs de projet). Les participants pitchent un problème ` +
  ...
  lines.push(`- **Thème** : ${EDITION.theme}`);
  lines.push(`- **Quoi** : 54 heures pour créer une startup en équipe, du pitch au prototype`);
  lines.push(`- **Quand** : ${dates}, du vendredi 18h30 au dimanche soir`);
